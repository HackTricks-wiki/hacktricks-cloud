...
The [AWS Aurora Data API CloudTrail guide](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/logging-using-cloudtrail-data-api.html) explicitly classifies `BatchExecuteStatement`, `BeginTransaction`, `CommitTransaction`, `ExecuteStatement`, and `RollbackTransaction` as **opt-in data events** on `AWS::RDS::DBCluster` for Aurora Serverless v2 and provisioned clusters. The [AWS Database Blog](https://aws.amazon.com/blogs/database/introducing-the-data-api-for-amazon-aurora-serverless-v2-and-amazon-aurora-provisioned-clusters/) explains that legacy Aurora Serverless v1 instead emitted Data API calls as default management events. Current CloudTrail management-event documentation still describes that v1 behavior, while the Aurora guide describes the newer data-event behavior. Its `ExecuteStatement` example redacts the SQL text, database, and schema. The previous book said all RDS calls were management events and placed `ExecuteStatement` in a default-management-event table. That was incomplete for newer Aurora generations. RDS control-plane API calls remain default management events; direct database-protocol SQL is outside CloudTrail and requires engine auditing for statement visibility.
