...
- [ ] Live-fire `enableRbacAuthorization` flip: as an identity with ONLY `vaults/write` (no data
      actions), toggle to RBAC, self-assign a KV data role, read a secret — confirm the full chain
      and the exact Activity Log events emitted. (cheap; premium not needed)
