...
- [ ] Retest `WEBSITE_RUN_FROM_PACKAGE=<URL>` on Linux Consumption with a minimal package that matches
      the configured runtime. Microsoft documents URL as the only supported Linux Consumption form; the
      prior HTTP 503 is inconclusive until package/runtime compatibility is controlled.
