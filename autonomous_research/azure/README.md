...
| 2026-09-25 | Cosmos DB | Resource-token persistence **vs master-key rotation** | **WORKS — new finding** — a minted `Read` resource token kept reading after primary rotated 3× + secondary 1× (rotations confirmed landing). Master-key rotation does NOT evict resource-token footholds; they live to TTL (≤5h). Corrected the wiki's wrong "rotation invalidates tokens" note. | Yes (RG deleted) |
