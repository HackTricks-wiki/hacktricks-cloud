...
| 2026-09-25 | Confidential Ledger | `ledgers/write` control-plane→data-plane self-grant (full chain incl. data-plane use) | **WORKS — was UNVERIFIED** — created a ledger naming our SP `Administrator` via control-plane `ledgers/write` only (no Azure RBAC: `role assignment list`=empty, token `roles:None`), then drove the data plane with just an Entra token + the ledger's CCF TLS cert: `POST /app/transactions` → 200 (tx 2.43), read back intact. Update path also confirmed (added a 2nd principal to the running ledger). Prior note verified only the control-plane mutation; added the data-plane half + recipe. | Yes (RG deleted; ~20 min under gate) |
| 2026-09-25 | Stream Analytics | Is `functions/RetrieveDefaultDefinition/action` a real job-less server-side SSRF? | **WORKS — was DOC-ONLY (and prior wiki NOTE said "unconfirmed")** — calling RetrieveDefaultDefinition with a `Microsoft.MachineLearningServices` binding whose `endpoint` is an arbitrary attacker HTTPS URL made the SA service open a TLS connection to it; the hit came from a **Microsoft-owned IP** (`57.152.0.0/16`, `MICROSOFT-MAINT`), not the tenant, correlated to the second, twice. No job created/started (no SUs). Constraints found + corrected: **http rejected** (https-only); the **classic `Microsoft.MachineLearning/WebService` binding is host-allowlisted to `services.azureml.net`** (SSRF blocked) — only the ML-Services binding is open; blind relay (no job-MI token). Upgraded the wiki NOTE to a lab-verified TIP. | Yes (RG deleted) |
| 2026-09-26 | AI Search | `searchServices/write` key-auth restoration and Activity Log | **WORKS** — a full-resource PUT changed `disableLocalAuth` from true to false; a previously valid admin key went 401→200. A minimal PATCH containing false returned success but left the property true, and the CLI update sent conflicting `authOptions`. `searchServices/write` and `listAdminKeys/action` appeared in Administrative Activity Log after ingestion delay. Network reopening could not be checked on Free tier. | Yes (all test RGs deleted and absence confirmed) |
| 2026-09-26 | App Configuration | Immutable snapshot loot persistence after deleting the live key | **WORKS** — a read-write store key created a snapshot of a fake secret; after the live key was deleted, the normal query returned zero items but the snapshot still returned the old value. Snapshot create/read and key deletion produced no Activity Log event (data plane; diagnostics off by default). | Yes (RG deleted, soft-deleted store purged, both inventories confirmed empty) |
| 2026-09-26 | Container Apps Jobs | Execution-template override with a known `secretRef` and available managed identity | **CANT-TEST** — the disposable consumption-only managed environment remained `Waiting` and its control-plane write later failed, so no Job, secret, identity, or execution could be created. The public entry is limited to Microsoft's explicit Jobs warning and Start API schema; an end-to-end minimum-permission retry remains queued. | Yes (failed environment and RG deletion confirmed) |
| 2026-09-26 | Storage | HNS ABAC Blob-vs-DFS path-condition differential | **REFUTED** — under one conditional Blob Data Reader assignment, both endpoints returned the allowed canary, denied a sibling and prefix-confusion path, and enforced scoped-list prefixes. Explicit `StorageRead` diagnostics captured equivalent ABAC grants/denials. Role removal took about 100 seconds to reach both endpoints symmetrically. | Yes (role removed; RG and all child resources confirmed gone) |
| 2026-09-26 | App Service Domains | Does catalogued `domains/read` authorize transfer-out, and what permission really does? | **Reader REFUTED / wildcard AUTHORIZATION WORKS / real-domain response DOC-ONLY** — read-only role got 403 naming hidden `domains/transferOut/write`; that exact action was unassignable, while `domains/*` with every published mutation excluded reached the provider. Documented real response contains the EPP `authCode`; no real domain was touched. | Yes (ACI, UAMI, roles, RG deleted; provider restored to Unregistered) |
| 2026-09-26 | Network DSCP Configuration | Does published singular `dscpConfiguration/read` authorize create/update? | **REFUTED** — list succeeded, but item GET and PUT required unpublished plural `dscpConfigurations/read` and `/write` respectively. The catalog is misleading, but Reader cannot mutate DSCP configuration through it. | Yes (ACI, UAMI, role, RG deleted; inventories empty) |
