...
- Removed two exact category duplicates from the privesc page: `tasks.get/list` secret harvesting and queue pause/purge/delete DoS remain in the post-exploitation page, where they belong. Retained `tasks.run` in privesc because replaying an existing task executes as its already-attached service account without requiring the caller to `actAs` that identity.
