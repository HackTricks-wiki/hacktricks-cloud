...
- [ ] Establish an owner control on a verified named PostgreSQL user. The owner failed with the documented two-permission custom role for the secret identity and four instance-ID shapes. Try `roles/cloudsql.admin` on the **control secret only** as a diagnostic; if that works, isolate the missing permission or role-binding issue before publishing any technique.
