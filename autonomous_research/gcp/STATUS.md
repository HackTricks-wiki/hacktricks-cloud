...
- Added the missing Cloud Tasks enum page with location-wide queue discovery, queue configuration/IAM/CMEK inspection, task `BASIC`/`FULL` inspection, security-review cues, logging visibility, and links to privesc, post-exploitation, persistence, and unauthenticated-access pages. Live read-only checks confirmed the location field and queue activity-log identifiers; no resource or configuration was changed. See `cloud-tasks/tested.md`.
