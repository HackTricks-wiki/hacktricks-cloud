    ...
    self.onmessage = async ({data}) => {
      if(data.type === 'init'){
        try {
          ...
          const main = await loadWithPolicy(mainSources, localCandidates('/searchindex-book.js', lang), privateHost);
          if(main) built.push(buildIndex(main, false));
          const cloud = await loadWithPolicy(cloudSources, localCandidates('/searchindex.js', lang), privateHost);
