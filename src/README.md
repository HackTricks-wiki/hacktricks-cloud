...
以下のワークフローは、Git に記載されている `clone`、`checkout`、`pull` 操作、およびリポジトリで公開されている言語ブランチとコンテナ設定に従います。<sup>[[2]](#references)[[3]](#references)[[8]](#references)[[9]](#references)[[10]](#references)</sup>
```bash
...
1 分後、ローカルの HackTricks Cloud に **[http://localhost:3377](http://localhost:3377)** からアクセスできるようになります。<sup>[[2]](#references)</sup>

また、Docker Compose がある場合は、repository のルートで次を実行します。<sup>[[2]](#references)[[6]](#references)</sup>
```bash
docker compose up
```
バンドルされた `docker-compose.yml` は、現在チェックアウトされているブランチをライブリロード付きで [http://localhost:3377](http://localhost:3377) に提供します。<sup>[[2]](#references)[[6]](#references)[[7]](#references)</sup>

### **Pentesting CI/CD Methodology**

**HackTricks CI/CD Methodology では、CI/CD アクティビティに関連するインフラストラクチャを pentest する方法を説明しています。** **導入**については、次のページをお読みください。<sup>[[11]](#references)</sup>

[pentesting-ci-cd-methodology.md](pentesting-ci-cd/pentesting-ci-cd-methodology.md)

### Pentesting Cloud Methodology

**HackTricks Cloud Methodology では、cloud 環境を pentest する方法を説明しています。** **導入**については、次のページをお読みください。<sup>[[12]](#references)</sup>
